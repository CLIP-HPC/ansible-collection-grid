...
@pytest.mark.parametrize(
    "name", ["/etc/grid-security/hostcert.pem", "/etc/yum.repos.d/UMD-4-updates.repo"]
)
...
def test_verify_ca(host):
    cmd = host.run(
        "/usr/bin/openssl verify -CApath /etc/grid-security/certificates /etc/grid-security/hostcert.pem"
    )  # noqa: E501
